...
export default function DashboardWebNuova() {
  const [groupedShifts, setGroupedShifts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [refresh, setRefresh] = useState(0);
  // Non usiamo più drawerOpen perché il drawer sarà fisso
  ...
  const fetchCommesse = useCallback(async () => {
    console.log('fetchCommesse iniziato');
    ...
    try {
      ...
      console.log('Chiamata API commesse...');
      // Ottieni tutte le commesse attive
      const commesseRes = await api.get('/api/commesse', {
        headers: { Authorization: `Bearer ${token}` }
      });
      console.log('Risposta API commesse ricevuta:', commesseRes.data);
      
      // Per ogni commessa, carica i task associati
      console.log('Inizio caricamento tasks per commesse');
      const commesseWithTasks = await Promise.all(
        commesseRes.data.map(async (commessa) => {
          try {
            console.log(`Caricamento tasks per commessa ${commessa.id}...`);
            const tasksRes = await api.get(`/api/commesse/${commessa.id}/tasks`, {
              headers: { Authorization: `Bearer ${token}` }
            });
            console.log(`Tasks per commessa ${commessa.id} ricevuti:`, tasksRes.data);
            return {
              ...commessa,
              tasks: tasksRes.data || []
            };
      ...
      console.log('Tutte le commesse con tasks caricate:', commesseWithTasks);
      ...
      if (selectedCommesse.length === 0 && commesseWithTasks.length > 0) {
        console.log('Selezione automatica di tutte le commesse');
        setSelectedCommesse(commesseWithTasks.map(c => c.id));
      }
      
    } catch (err) {
      console.error('Errore nel caricamento delle commesse:', err);
      setCommesseError('Errore nel caricamento delle commesse: ' + (err.message || err));
      setCommesse([]);
    } finally {
      setCommesseLoading(false);
      console.log('fetchCommesse completato');
    }
  ...
  async function fetchShifts(start, end) {
    console.log('fetchShifts iniziato con date:', start, end);
    setLoading(true);
    setError('');
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      if (!token) {
        console.log('Token non trovato in fetchShifts');
      ...
      console.log('Chiamata API shifts...');
  ...
  const handleDatesSet = useCallback((arg) => {
    console.log('handleDatesSet chiamato', arg);
    try {
      // Validazione dell'oggetto arg e delle date
      if (!arg || typeof arg !== 'object') {
        console.log('handleDatesSet: arg non valido', arg);
        return; // Aggiunto return esplicito
      }
      ...
      if (!startStr || !endStr) {
        console.error('Date non valide in handleDatesSet', { startStr, endStr });
        return;
      }
      
      console.log('handleDatesSet: date valide', { startStr, endStr });
  ...
  return (
    <ErrorBoundary>
      <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: '#f5f6fa' }}>
        {/* AppBar con menu */}
        <AppBar position="fixed" sx={{ zIndex: 1201, background: '#fff', color: '#222', boxShadow: '0 1px 6px 0 #e0e0e0', width: { sm: `calc(100% - 340px)` }, ml: { sm: '340px' } }}>
          <Toolbar>
